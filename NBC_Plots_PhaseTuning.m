...
[ ~, binInd ] = histc(whiskerPhase, edges); 
    
colormapSeed=lines;
cmap=[colormapSeed(1:7,:);(colormapSeed+flipud(colormap(copper)))/2;autumn];
phaseTuning=nan(size(ephysData,1),1);

for unitNum=1:size(ephysData,1)
    clearvars unitSpikeRate 
    % Data masking
    unitSpikeRate= ephysData(unitNum,dataMask);
...
%         end
    ...
    if size(ephysData,1)>1
        subplot(ceil(size(ephysData,1)/4),4,unitNum)
    end
